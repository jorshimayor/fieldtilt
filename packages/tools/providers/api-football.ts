 ...
 * Chelsea team id: 49 · Premier League id: 39
 *
...
function mapFixture(r: any): NormalizedFixture {
  ...
  return {
    ...
    opponentId: (isChelseaHome ? r?.teams?.away?.id : r?.teams?.home?.id) ?? null,
    status,
    goalsHome,
    goalsAway,
    outcome,
...
export const apiFootballProvider: FootballProvider = {
  ...
  async getLiveMatch() {
    ...
    const game = (json.response || []).find(
      (e: any) => e?.teams?.home?.id === CHELSEA_TEAM_ID || e?.teams?.away?.id === CHELSEA_TEAM_ID
    );
    if (!game) {
  ...
  async getMatchStats(fixtureId) {
    ...
    const json = await af<any>(path);
    const chelseaEntry = (json.response || []).find((e: any) => e?.team?.id === CHELSEA_TEAM_ID);
    const stats = chelseaEntry?.statistics || [];
    const data: NormalizedMatchStats = {
      fixtureId,
      team: chelseaEntry ? "home" : "unknown",
  ...
  async getHeadToHead(ref) {
    ...
    const last = 10;
    const path = `/fixtures/headtohead?h2h=${CHELSEA_TEAM_ID}-${opponentId}&last=${last}`;
    ...
    for (const r of json.response || []) {
      ...
      const ours = chelseaHome ? r?.goals?.home : r?.goals?.away;
      const theirs = chelseaHome ? r?.goals?.away : r?.goals?.home;
